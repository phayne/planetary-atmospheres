...
########
# --------------------------------------------
# Equilibrium vapor pressure over solid N2
# Brown and Ziegler (1980)
# --------------------------------------------
# Input:
#    T = temperature of solid [K]
...
def pn2(T):
    ...
    # Pressure in torr
    ptorr = np.exp(A0 + A1/T + A2/T**2 + A3/T**3 + A4/T**4)
...
def n2SublimationRate(T):
    # Equilibrium water vapor pressure over ice
    pv = pn2(T) # [Pa]

    # Sublimation rate
    mdot = pv * np.sqrt( mw_n2/(2*pi*R*T) ) # [kg.m-2.s-1]
